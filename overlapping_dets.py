...
A = kneighbors_graph(X, 2, mode='distance', n_jobs=2)
A.toarray()
'''
...
def combine_CSVs(results_dir):
    ...
    for ch1, ch2 in zip(tifs[::2], tifs[1::2]):
        ...
        total_syn = 0
        total_size = 0
        # c = [cluster id, size, psd, syn, c_x, c_y]
...
def img_worker(ch1, ch2, results_dir, r_threshold, g_threshold):
    ...
    with open(csv_name, 'w') as csv_file:
        ...
        header = ['Cluster ID', 'Size', 'Mean PSD', 'Mean Syn1', 'Mean x', 'Mean y']
        ...
        while clusters:
            ...
            if c_type == 'normal':
                ...
                writer.writerow([num_rows, size, avg_psd, avg_syn, c_x, c_y])
                num_rows += 1
            elif c_type == 'edge':            
