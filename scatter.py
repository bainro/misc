...
class PosOverlayMixin(object):
    def __init__(self, *args, **kwargs):
        super(PosOverlayMixin, self).__init__(*args, **kwargs)
        self.state_attrs += ('overlay',)
        self.local_state_attrs += ()
        self._overlay = True

    def attach(self, gui):
        super(PosOverlayMixin, self).attach(gui)
        self.actions.add(self.toggle_overlayed)
        self.actions.separator()

    def toggle_overlayed(self):
...
class ScatterView(PosOverlayMixin, MarkerSizeMixin, LassoMixin, ManualClusteringView):
    ...
    def get_clusters_data(self, load_all=None):
        ...
        if not load_all:
            bunchs = self.coords(self.cluster_ids, overlay=self._overlay) or ()
        elif 'load_all' in inspect.signature(self.coords).parameters:
            bunchs = self.coords(self.cluster_ids, load_all=load_all, overlay=self._overlay) or ()
        else:
            logger.warning(
                "The view `%s` may not load all spikes when using the lasso for splitting.",
                self.__class__.__name__)
            bunchs = self.coords(self.cluster_ids, overlay=self._overlay)
